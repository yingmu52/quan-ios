...
- (void)testUploadDeviceToken{
    
    XCTestExpectation *expectation = [self expectationWithDescription:@"消息推送时向后台发送Device Token"];
    NSString *fakeToken = @"d1e0db6cdf9158602f20f89e0922e171604873e9bc7f2e4e485856297e743c7b";
    [self.fetchCenter sendDeviceToken:fakeToken completion:^{
        [expectation fulfill];
    }];

    [self waitForExpectationsWithTimeout:expectationTimeout
                                 handler:^(NSError * _Nullable error) {
                                     XCTAssertNil(error,@"消息推送时向后台发送Device Token错误");
                                 }];
